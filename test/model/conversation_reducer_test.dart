...
void main() {
  test('RunFinishedEvent with stopReason cancelled sets a cancelled outcome',
    ...
    expect(reducer.current.sessionState.runOutcome, RunOutcome.cancelled);
  });
  ...
  test(
    ...
    expect(reducer.current.sessionState.runOutcome, RunOutcome.interrupted);
  });
  ...
  group('text messages', () {
    test(
      ...
      expect((r.current.timeline.single as TextStreamTimelineItem).text,
          'Hello, ');
  ...
  group('reasoning messages', () {
    test(
      ...
      r.apply(const ReasoningMessageContentEvent(
          messageId: 'r1', delta: 'thinking...'));
      expect(r.current.timeline, hasLength(1));
      expect((r.current.timeline.single as TextStreamTimelineItem).text,
          'thinking...');
  ...
  group('permission/elicitation via state delta', () {
    group('elicitation (Adapter A: pocketcoder StateDelta)', () {
      test(
        ...
        expect(r.current.timeline, hasLength(1));
        final item =
            r.current.timeline.single as ElicitationRequestTimelineItem;
  ...
  group('resolveRequest', () {
    ...
    test('resolved-id set survives the cold-replay reset marker', () {
      ...
      expect(r.current.timeline, isEmpty,
          reason:
              'resolved-id set must survive _reset(), or reconnect resurrects resolved items');
    });
  });

  group(
      'permission/elicitation/tool-request (Adapter B: canonical CustomEvent)',
      () {
    ...
    test(
      ...
      expect(r.current.timeline, hasLength(2));
      expect(
          r.current.timeline.any(
              (i) => i is PermissionRequestTimelineItem && i.requestId == 'b1'),
          isTrue);
    });
  });
  ...
  group('deterministic identity', () {
    test(
      ...
      final r = ConversationReducer()
        ...
        ..apply(const ToolCallStartEvent(
            toolCallId: 'tc1', toolCallName: 'search'));
      expect(
          r.current.timeline.whereType<ToolCallTimelineItem>(), hasLength(1));
    });
    ...
    test(
      ...
      final r = ConversationReducer()
        ...
        ..apply(const TextMessageStartEvent(
            messageId: 'm1', role: TextMessageRole.assistant))
        ..apply(const ReasoningMessageContentEvent(
            messageId: 'm1', delta: 'thinking...'))
        ..apply(const TextMessageContentEvent(messageId: 'm1', delta: 'hello'));
      expect(
          r.current.timeline.whereType<TextStreamTimelineItem>(), hasLength(2));
    });
    ...
    test(
      ...
      () {
        ...
        expect(
            r.current.timeline.whereType<ToolCallTimelineItem>(), hasLength(1));
        expect(r.current.timeline.whereType<PermissionRequestTimelineItem>(),
            hasLength(1));
        ...
        expect(
          ...
          hasLength(1),
          reason:
              'resolving the permission destroyed the original tool call entry',
        );
        ...
        r.apply(const ToolCallResultEvent(
            messageId: 'm1', toolCallId: 'tc1', content: 'ok'));
        final toolCalls =
            r.current.timeline.whereType<ToolCallTimelineItem>().toList();
    ...
    test(
      ...
      expect(
          r.current.timeline.whereType<ToolCallTimelineItem>(), hasLength(1));
      expect(r.current.timeline.whereType<ToolRequestTimelineItem>(),
          hasLength(1));
